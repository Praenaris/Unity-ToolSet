...
namespace DragonResonance.Editor.Drawers
{
	...
	public class SavedataResourceDrawer : PropertyDrawer
	{
		...
		private const float ROOT_PADDING = 16f;
		...
		private static float _rootWidth_internal = -1f;	// Caching only, use the property instead
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			EditorGUI.BeginProperty(position, label, property);
			{
				...
				Rect rootRect = new(position.x, position.y, GetRootWidth(), lineHeight);
				Rect companyRect = new(rootRect.xMax + SPACING, position.y, companyWidth, lineHeight);
				...
				Rect fullPathRect = new(position.x, secondLineY, position.width, lineHeight);
				...
				EditorGUI.indentLevel = 0;
				{
					...
					EditorGUI.SelectableLabel(fullPathRect, GetFullPathPreview(resource), EditorStyles.miniLabel);
				}
		...
		private static string GetFullPathPreview(SavedataResource resource)
		{
			try {
		...
		private static float GetRootWidth()
		{
			if (_rootWidth_internal >= 0f) return _rootWidth_internal;

			foreach (string rootName in Enum.GetNames(typeof(SavedataRootPath)))
				_rootWidth_internal = Mathf.Max(_rootWidth_internal, EditorStyles.popup.CalcSize(new GUIContent(ObjectNames.NicifyVariableName(rootName))).x);
			_rootWidth_internal += ROOT_PADDING;
			return _rootWidth_internal;
		}
	}
}
